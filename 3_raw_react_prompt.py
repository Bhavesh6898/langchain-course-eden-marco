# Change 1: Add re + inspect - we'll pasrse tool calls from raw text instead of structured JSON.
import re
import inspect
...
def get_tool_description(tools_dict):
    ...
    for tool_name, tool_function in tools_dict.items():
        # ___wrapped___ bypasses decorator wrappers (eg., @traceable adds *, config=None)
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Change 6: Parse tool calls from raw text with rege - fragile if LLM doesn't change format.
